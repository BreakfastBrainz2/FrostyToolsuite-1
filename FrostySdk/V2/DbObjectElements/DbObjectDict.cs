...
public class DbObjectDict : DbObjectV2
{
    private readonly Dictionary<string, DbObjectV2> m_items;

    protected internal DbObjectDict(Type inType)
        : base(inType)
    {
        m_items = new Dictionary<string, DbObjectV2>();
    }

    protected internal  DbObjectDict(int inCapacity)
        : base(Type.Dict | Type.Anonymous)
    {
        m_items = new Dictionary<string, DbObjectV2>(inCapacity);
    }
    ...
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsDict() : defaultValue;
    }
    ...
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsList() : defaultValue;
    }

    public bool AsBoolean(string name, bool defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsBoolean() : defaultValue;
    }

    public string AsString(string name, string defaultValue = "")
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsString() : defaultValue;
    }

    public int AsInt(string name, int defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsInt() : defaultValue;
    }

    public uint AsUInt(string name, uint defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsUInt() : defaultValue;
    }

    public long AsLong(string name, long defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsLong() : defaultValue;
    }

    public ulong AsULong(string name, ulong defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsULong() : defaultValue;
    }

    public float AsFloat(string name, float defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsFloat() : defaultValue;
    }

    public double AsDouble(string name, double defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsDouble() : defaultValue;
    }

    public Guid AsGuid(string name, Guid defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsGuid() : defaultValue;
    }

    public Sha1 AsSha1(string name, Sha1 defaultValue = default)
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsSha1() : defaultValue;
    }
    ...
    {
        return m_items.TryGetValue(name, out DbObjectV2? item) ? item.AsBlob() : defaultValue;
    }
    ...
    protected override void InternalSerialize(DataStream stream)
    {
        ...
        {
            foreach (DbObjectV2 value in m_items.Values)
            {
