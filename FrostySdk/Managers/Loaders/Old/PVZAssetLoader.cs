...
namespace FrostySdk.Managers
{
    public partial class AssetManager
    {
        internal class PVZAssetLoader : IAssetLoader
        {
            ...
            {
                foreach (string sbName in parent.m_fileSystem.SuperBundles)
                //foreach (CatalogInfo catalog in parent.fs.EnumerateCatalogInfos())
                {
                    //foreach (string sbName in catalog.SuperBundles.Keys)
                    {
                        //SuperBundleEntry sbe = parent.superBundles.Find((SuperBundleEntry a) => { return a.Name == sbName; });
                        int sbIndex = -1;
                                ...
                                using (NativeReader reader = new NativeReader(new MemoryStream(buffer)))
                                {
                                    ...
                                    if (values[0] != 0)
                                    {
                                        for (int i = 0; i < values[2]; i++)
                                            reader.ReadInt(Endian.Big); //unkList.Add(reader.ReadInt(Endian.Big));
                                    ...
                                    if (values[3] != 0)
                                    {
                                        ...
                                        for (int i = 0; i < values[5]; i++)
                                            reader.ReadInt(Endian.Big); //unkValues.Add(reader.ReadInt(Endian.Big));
