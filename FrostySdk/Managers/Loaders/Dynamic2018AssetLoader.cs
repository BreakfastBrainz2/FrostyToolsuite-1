...
namespace FrostySdk.Managers.Loaders;
...
public class Dynamic2018AssetLoader : IAssetLoader
{
    ...
    private Code LoadSuperBundle(FileSystemSource inSource, SuperBundleInstallChunk inSbIc)
    {
        ...
        // load the toc
        DbObjectDict? toc = DbObjectV2.Deserialize(path)?.AsDict();
        ...
        if (toc.ContainsKey("bundles"))
        {
            ...
            else
            {
                ...
                foreach (DbObjectV2 obj in bundles)
                {
                    ...
                    else if (!isCas && isDelta)
                    {
                        ...
                        if (baseBundleMapping.TryGetValue(Utils.HashString(bundle.Name, true), out BaseBundleHelper helper))
                        {
                            baseSbPath ??= FileSystemManager.ResolvePath(false, $"{inSbIc.Name}.sb");
                            relBaseSbPath ??= Path.GetRelativePath(FileSystemManager.BasePath, baseSbPath);
                            baseSbStream ??=
        ...
        if (toc.ContainsKey("chunks"))
        {
            ...
            if (chunkList.Count > 0)
            {
                ...
                foreach (DbObjectV2 obj in chunkList)
                {
                ...
                if (inSource.Path == FileSystemSource.Patch.Path)
                {
                    ...
                    DbObjectDict? baseToc = string.IsNullOrEmpty(basePath) ? null : DbObjectV2.Deserialize(basePath)?.AsDict();
                    ...
                    if (baseToc is not null)
                    {
                        ...
                        foreach (DbObjectV2 obj in baseToc.AsList("chunks"))
                        {
                            DbObjectDict chunkObj = obj.AsDict();
    ...
    private Dictionary<int, BaseBundleHelper> LoadBaseBundles(string inPath)
    {
        ...
        DbObjectDict? toc = DbObjectV2.Deserialize(inPath)?.AsDict();
        ...
        foreach (DbObjectV2 obj in toc.AsList("bundles"))
        {
            string name = obj.AsDict().AsString("id");
            retVal.Add(Utils.HashString(name, true), new BaseBundleHelper(obj.AsDict().AsLong("offset"), obj.AsDict().AsLong("size")));
        }
    ...
    {
        DbObjectDict? bundleObj = DbObjectV2.Deserialize(stream)?.AsDict();
