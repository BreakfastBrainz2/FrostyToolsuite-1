...
namespace AssetBankPlugin.Ant
{
    public partial class VbrAnimationAsset : AnimationAsset
    {
        ...
        public List<Vector4> Decompress()
        {
            ...
            int headerSize = Data.Length - totalFrameBlockSize;

            //  BFN FIX DataOffset bug to detect corrupted 16-bit palette assets
            int buggyDataOffset = KeyTimeSize + constDofCount + ConstChanMapSize + (channelCount * 4) + VectorOffsetSize + FloatOffsetSize;

            if (buggyDataOffset != headerSize)
            {
                // 1. Undo the EA compiler's buggy swap to restore the payload to pristine Big-Endian
                int currentOffset = buggyDataOffset;
                for (int i = 0; i < FrameBlockSize; i++)
                {
                    int siz = FrameBlockSizes[i];
                    int start = currentOffset + 3;
                    int len = siz - 3;
                    if (start + len <= Data.Length && len > 0)
                    {
                        Array.Reverse(Data, start, len);
                    }
                    currentOffset += siz;
                }

                // 2. Apply the correct Little-Endian swap starting at the true header offset
            ...
            byte[] fData = new byte[totalFrameBlockSize];
            byte[] MetaData = new byte[headerSize];
            Array.Copy(Data, headerSize, fData, 0, fData.Length);
            Array.Copy(Data, 0, MetaData, 0, headerSize);

            int pos = 0;
            pos += KeyTimeSize;

            bool use8BitIndices = ConstPaletteSize <= 256;
            int indexStride = use8BitIndices ? 1 : 2;
            int totalIndexBytes = constDofCount * indexStride;

            PaletteIndexes = new ushort[constDofCount];
            if (use8BitIndices)
            {
                for (int i = 0; i < constDofCount; i++)
                    PaletteIndexes[i] = MetaData[pos + i];
            }
            else
            {
                for (int i = 0; i < constDofCount; i++)
                    PaletteIndexes[i] = BitConverter.ToUInt16(MetaData, pos + i * 2);
            }
            pos += totalIndexBytes;

            ConstChanMap = new byte[ConstChanMapSize];
            if (ConstChanMapSize > 0)
            {
                Array.Copy(MetaData, pos, ConstChanMap, 0, ConstChanMapSize);
                pos += ConstChanMapSize;
            }

            int numBitsSize2 = channelCount * 4;
            byte[] numbitsByte = new byte[numBitsSize2];
            Array.Copy(MetaData, pos, numbitsByte, 0, numBitsSize2);
            pos += numBitsSize2;

            VectorOffsets = new byte[VectorOffsetSize + FloatOffsetSize];
            if (VectorOffsetSize + FloatOffsetSize > 0)
            {
                Array.Copy(MetaData, pos, VectorOffsets, 0, VectorOffsets.Length);
                pos += VectorOffsets.Length;
            }
