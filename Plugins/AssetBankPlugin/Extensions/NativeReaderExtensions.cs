...
namespace AssetBankPlugin.Extensions
{
    public static class NativeReaderExtensions
    {
        ...
        public static long ReadReference(this NativeReader r, Endian endian)
        {
            ...
            return (absolute < 0 || absolute > r.BaseStream.Length) ? 0 : absolute;
        }
        ...
        public static void ReadDataHeader(this NativeReader r, Endian bigEndian,
            out uint hash, out uint type, out uint offset)
        {
        ...
        public static byte[] ReadByteArray(this NativeReader r, int count) => r.ReadBytes(count);
        ...
        public static short[] ReadShortArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                for (int i = 0; i < count; i++) { int b = i * 2; array[i] = (short)(raw[b] | raw[b + 1] << 8); }
            else
                for (int i = 0; i < count; i++) { int b = i * 2; array[i] = (short)(raw[b + 1] | raw[b] << 8); }
            return array;
        }
        ...
        public static ushort[] ReadUShortArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                for (int i = 0; i < count; i++) { int b = i * 2; array[i] = (ushort)(raw[b] | raw[b + 1] << 8); }
            else
                for (int i = 0; i < count; i++) { int b = i * 2; array[i] = (ushort)(raw[b + 1] | raw[b] << 8); }
            return array;
        }
        ...
        public static int[] ReadIntArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                for (int i = 0; i < count; i++) { int b = i * 4; array[i] = raw[b] | raw[b + 1] << 8 | raw[b + 2] << 16 | raw[b + 3] << 24; }
            else
                for (int i = 0; i < count; i++) { int b = i * 4; array[i] = raw[b + 3] | raw[b + 2] << 8 | raw[b + 1] << 16 | raw[b] << 24; }
            return array;
        }
        ...
        public static uint[] ReadUIntArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                for (int i = 0; i < count; i++) { int b = i * 4; array[i] = (uint)(raw[b] | raw[b + 1] << 8 | raw[b + 2] << 16 | raw[b + 3] << 24); }
            else
                for (int i = 0; i < count; i++) { int b = i * 4; array[i] = (uint)(raw[b + 3] | raw[b + 2] << 8 | raw[b + 1] << 16 | raw[b] << 24); }
            return array;
        }
        ...
        public static long[] ReadLongArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                ...
                {
                    int b = i * 8;
                    array[i] = (long)(uint)(raw[b + 4] | raw[b + 5] << 8 | raw[b + 6] << 16 | raw[b + 7] << 24) << 32 |
                ...
                {
                    int b = i * 8;
                    array[i] = (long)(uint)(raw[b + 3] | raw[b + 2] << 8 | raw[b + 1] << 16 | raw[b] << 24) << 32 |
        ...
        public static ulong[] ReadULongArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                ...
                {
                    int b = i * 8;
                    array[i] = (ulong)(uint)(raw[b + 4] | raw[b + 5] << 8 | raw[b + 6] << 16 | raw[b + 7] << 24) << 32 |
                ...
                {
                    int b = i * 8;
                    array[i] = (ulong)(uint)(raw[b + 3] | raw[b + 2] << 8 | raw[b + 1] << 16 | raw[b] << 24) << 32 |
        ...
        public static float[] ReadSingleArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                ...
                {
                    int b = i * 4;
                    array[i] = new FloatUnion { UIntValue = (uint)(raw[b] | raw[b + 1] << 8 | raw[b + 2] << 16 | raw[b + 3] << 24) }.FloatValue;
                }
            else
                for (int i = 0; i < count; i++)
                {
                    int b = i * 4;
                    array[i] = new FloatUnion { UIntValue = (uint)(raw[b + 3] | raw[b + 2] << 8 | raw[b + 1] << 16 | raw[b] << 24) }.FloatValue;
                }
            return array;
        }
        ...
        public static double[] ReadDoubleArray(this NativeReader r, int count, Endian endian)
        {
            ...
            if (endian == Endian.Little)
                for (int i = 0; i < count; i++)
                {
                    ...
                    array[i] = new DoubleUnion { ULongValue = ((ulong)hi << 32) | lo }.DoubleValue;
                }
            else
                for (int i = 0; i < count; i++)
                {
