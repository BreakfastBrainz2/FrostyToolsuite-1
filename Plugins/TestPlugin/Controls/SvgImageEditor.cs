...
namespace TestPlugin.Controls
{
    ...
    // This editor is used to edit assets of type SvgImage, it is instantiated from the GetEditor
    // function of the corresponding AssetDefinition
    [TemplatePart(Name = PART_Canvas, Type = typeof(Viewbox))]
    public class SvgImageEditor : FrostyAssetEditor
    {
        private const string PART_Canvas = "PART_Canvas";

        #region -- GridVisible --
        public static readonly DependencyProperty GridVisibleProperty = DependencyProperty.Register("GridVisible", typeof(bool), typeof(SvgImageEditor), new FrameworkPropertyMetadata(true));
        ...
        static SvgImageEditor()
        {
            DefaultStyleKeyProperty.OverrideMetadata(typeof(SvgImageEditor), new FrameworkPropertyMetadata(typeof(SvgImageEditor)));
        }

        public SvgImageEditor(ILogger inLogger)
            : base(inLogger)
        {
        }
        ...
        {
            SvgImageAsset loadedAsset = App.AssetManager.GetEbxAs<SvgImageAsset>(entry);
            return loadedAsset;
        }

        // imports a SVG file and replaces the asset with the new data
            ...
            if (ofd.ShowDialog())
            {
                ...
                FrostyTaskWindow.Show("Importing SVG", "", (task) =>
                {
                    try
                    {
                        ...
                        // create new SVG image resource
                        var image = new SvgImageResource(width, height);

                        // populate resoruce with path data
                        IterateNodesAndCreatePaths(image, xmlDoc["svg"], ref warningCount);

                        // modify the asset with the new SVG image resource
                        SvgImageAsset svgAsset = asset as SvgImageAsset;
                        asset.ApplyModifiedResource(new ModifiedSvgImageAsset(image));

                        // send up modifications to the asset manager
                ...
                if (!bFailed)
                {
                    // if everything went smoothly, advise editor of modifications
        ...
        private void IterateNodesAndCreatePaths(SvgImageResource image, XmlNode parentNode, ref int warningCount)
        {
            foreach (XmlNode node in parentNode)
            {
                if (node is XmlElement subElem)
                {
                    if (subElem.Name == "g")
                    {
                        IterateNodesAndCreatePaths(image, subElem, ref warningCount);
                    }
